...
class TestApiEndpoints(unittest.TestCase):
    ...
    def test_api_can_add_book(self):
        ...
        response = self.client().post('/api/v1/books', 
                                      data=json.dumps({"book_id":16, "title": "Queer cats, An African Tale",
                                                       "author": "Chimamano Nakote"}), content_type='application/json')
        print(response.data)
    ...
    def test_book_can_be_edited(self):
        ...
        res = self.client().put('/api/v1/books/2', data=json.dumps({"title":"The fault in our stars",
                                                         "author":"John Greene",
                                                         "edition": "7th"}), content_type='application/json')
    ...
    def test_user_actions(self):
        ...
        result = self.client().post('/api/v1/auth/register', content_type='application/json',
                                    ...
                                    "email":"hawa@gma.com", "password":"where", "confirm_password":"where"}))
    ...
    def test_reset_password(self):
        result = self.client().post('/api/v1/auth/register', content_type='application/json',
                                    data=json.dumps({"username": "lucy", "name": "Morningstar",
                                                     "email": "lucy@hot.com", "password": "1234", "confirm_password": "1234"}))
        self.assertEqual(result.status_code, 200)
